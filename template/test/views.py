...
class DevalarmBase(BaseView):

    def __init__(self):
        super(DevalarmBase, self).__init__()
...
class QueryDevalarm(DevalarmBase):
    """
    ...
    def __init__(self):
        super(QueryDevalarm, self).__init__()

    def main_uery(self, args):
        query_sql = r"""select * from wd_devalarm """
        query_where_list = []
        if args.get('se_key', '') != '':
    ...
    def views(self):
        ...
        if args.get('type', '') == '':
            ...
            query_sql = r"""select * from wd_devalarm  where id={}""".format(args.get('id'))
        result = self._db.query(query_sql)
        for index, item in enumerate(result):
            
            
            if 'AlarmTime' in item.keys():
                item['AlarmTime'] = datetime.datetime.strftime(item['AlarmTime'], "%Y-%m-%d %H:%M:%S")
...
class CreateDevalarm(DevalarmBase):
    """
        请求参数：{
    ...
    def __init__(self):
        super(CreateDevalarm, self).__init__()
    ...
    def views(self):
        ...
        if args.get('id', '') == '' or args.get('id') is None:
            # create
            insert_sql = r"""insert into wd_devalarm({}) value ({});""".format(*self.get_create_sql(form))
            self._db.insert(insert_sql)
        else:
            # edit
            update_sql = r"""update wd_devalarm set {} where id={};""".format(
...
class RemoveDevalarm(DevalarmBase):
    ...
    def __init__(self):
        super(RemoveDevalarm, self).__init__()

    def views(self):
        args = request.get_json()
        if args.get('id', '') == '':
            return jsonify(status_code.CONTENT_IS_NULL)
        delete_sql = r"""delete from wd_devalarm where id = {};""".format(args.get('id'))
        try:
